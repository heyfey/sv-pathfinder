...
// Status: "<total> distinct nets (<named> named), <devices> devices". total = distinct nets incl.
// unnamed, named = the wire-label index. fifo has unnamed internal nets, so total must exceed named.
const m = r.status.match(/(\d+) distinct nets \((\d+) named\), \d+ devices/);
const ok = report('smoke', [
  ['renders', r.rendered === true],
  ['status shows "<total> distinct nets (<named> named), <devices> devices"', !!m],
